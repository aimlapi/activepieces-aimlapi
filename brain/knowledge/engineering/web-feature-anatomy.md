...
On any query that fetches a page's **primary** data — the table rows, the list, the thing the page exists to show — set `meta: { showErrorToast: true, loadSubsetOptions: {} }` (`meta` is typed as `QueryCollectionMeta`, which makes `loadSubsetOptions` mandatory — the flag alone does not compile). `QueryCache.onError` in `app/query-client.ts` turns that into a red toast — one toast, once per outage, and only while the query still has an active observer. Three guards, and each one is there because the obvious version of it leaks:
...
The error payload goes to `console.error` and into a `<details>` disclosure on the toast (`components/custom/query-error-toast.tsx`), collapsed by default. Opening it re-renders the same toast id with `duration: Infinity` plus a close button, so the JSON cannot expire while it is being read; collapsing it hands the toast back its five seconds. Leave it off for auxiliary queries (feature flags, piece metadata, single-item fetches, filter options, user details) — those should fail silently.
